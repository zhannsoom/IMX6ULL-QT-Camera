...
#define FRAMEBUFFER_COUNT   3               //帧缓冲数�?

/*** 摄像头像素格式及其描述信�? ***/
typedef struct camera_format {
    unsigned char description[32];  //字符串描述信�?
...
static int v4l2_set_format(void)
{
    ...
    fmt.fmt.pix.width = width;  //视频帧宽�?
    fmt.fmt.pix.height = height;//视频帧高�?
    ...
    for ( ; ; ) {
        ...
        for(buf.index = 0; buf.index < FRAMEBUFFER_COUNT; buf.index++) {
            ...
            for (j = 0, base=screen_base, start=buf_infos[buf.index].start;
                ...
                memcpy(base, start, min_w * 2); //RGB565 一个像素占2个字�?
                base += width;  //LCD显示指向下一�?
                start += frm_width;//指向下一行数�?
            }
...
int main(int argc, char *argv[])
{
    ...
    /* 读取数据：出�? */
    v4l2_read_data();       //在函数内循环采集数据、将其显示到LCD�?
